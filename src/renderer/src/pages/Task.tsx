...
import { IconArrowLeft, IconBrandChrome, IconCheck, IconChevronDown, IconDeviceMobile, IconDots, IconPencilMinus, IconPlus } from '@tabler/icons-react';
    ...
    <div className='w-80 border-r  h-full py-2 max-h-full flex flex-col overflow-hidden'>
      <div className="px-4">
          <div className='flex items-center gap-1 '>
            {
              onEditAppName ?
                <div className='flex items-centerr gap-2 justify-between w-full'>
                  <input onChange={(e) => setAppName(e.target.value)} type="text" value={appName} className='w-full p-2 border outline-none focus:border-sky-500 rounded-lg' />
...
function Flow({ innitialNodes, innitialEdges, callbackNodes, callbackEdges }) {
  ...
  const [selectedNodes, setSelectedNodes] = useState<Node['id'][]>([]);
  const [selectedEdges, setSelectedEdges] = useState(null);
  const [pasteNodes, setPasteNodes] = useState<Node[]>([]);
  const [customSelectedNodes, setCustomSelectNodes] = useState<Node[]>([]);
...
export default function Task({ task, onSave }) {
  const [nodes, setNodes] = useState<Node[]>(task.nodes)
  const [edges, setEdges] = useState(task.edges)
  const [appName, setAppName] = useState(task.title)
  const [platform, setPlatform] = useState<IPlatform>(task.platform)
  const handleOnSave = () => {
    const data = {
      title: appName,
      nodes: nodes,
